...
* 2.2.1 ``onFulfilled`` 和 ``onRejected`` 两者是可选
    * 1 如果 onFulfilled 不是一个函数, 将被忽略.
    * 2 如果 onRejected 不是一个函数, 将被忽略.

* 2.2.2 如果 ``onFulfilled`` 是一个函数
    * 1 onFulfilled 是在 promise 状态为 fulfilled 之后调用的，且 ``promise`` 返回结果值是作为 ``onFulfilled`` 第一个参数。
    * 2 onFulfilled 只有在 promise 状态 fulfilled 后才会被调用。
    * 3 onFulfilled 最多只会被调用一次。

* 2.2.3 如果 ``onRejected`` 是一个函数
    * 1 onRejected 只会在 promise 状态为 rejected 之后才会被调用，且 ``promise`` 返回结果值是作为 ``onFulfilled`` 第一个参数。
    * 2 onRejected 只有在 promise 状态 rejected 后才会被调用。
    * 3 onRejected 最多只会被调用一次。
...
* 2.2.7 ``then`` 必须返回一个新的 promise
    ...
    * 1 如果 ``onFulfilled`` 或 ``onRejected`` 返回一个值 x，则运行 Promise 解决过程 [[Resolve]](promise2, x)[2.3]。
    * 2 如果 ``onFulfilled`` 或 ``onRejected`` 中的任何一个抛出异常 e，那么 promise2 状态为 rejected 且错误原因为 e。
    * 3 如果 ``onFulfilled`` 不是一个函数且 promise1 是状态为 fulfilled, promise2 状态也必须是 fulfilled 并且接收值与 promise1 一样. 
    * 4 如果 ``onRejected`` 不是一个函数 function 且 promise1 状态为 rejected, promise2 状态也必须是 rejected ，且与 promise1 错误原因一致。
...
* 1 如果 ``promise`` 和 ``x`` 指向的是同一个对象，则以 TypeError 作为原因拒绝 ``promise``。
* 2 如果 ``x`` 是一个 promise， 采用 x 对应的状态：
    * 1 如果 x 处于 pending，那么 promise 必须保持 pending 状态，直到 x 被 fulfilled 或 rejected。
    * 2 如果当 x 状态为 fulfilled 时，以相同的值实现 promise。
    * 3 如果当 x 被拒绝时，以相同的原因拒绝 promise。
* 3 否则，如果 x 是一个对象或函数
    * 1 将 then 设置为 x.then。[3.5]

    * 2 如果获取属性 x.then 的结果是抛出异常 e，则以 e 为原因拒绝承诺。
    * 3 如果 then 是一个函数，就以 x 作为 this，第一个参数为 resolvePromise，第二个参数为 rejectPromise 来调用它，其中：
...
* 4 如果 x 不是一个对象或函数，x 作为 promise 成功的值。
